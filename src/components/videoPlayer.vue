...
onMounted(async () => {
  ...
  const videoSrc = videoData.video_url;// 'https://zztraffic.upera.tv/3025234-0-hls.m3u8?ref=uVhD';
  trackSrc.value = videoData.subtitle.replace("https://s600.upera.tv/s3/files/","/subtitles-proxy/");
  if (video) {
    if (Hls.isSupported()) {
      const hls = new Hls();
